...
export function _PriceListItemsTable({ priceListId, items }: PriceListItemsTableProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success('Ítem eliminado de la lista');
      router.refresh();
    } catch (error) {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      toast.success(`${count} ítem${count !== 1 ? 's' : ''} eliminado${count !== 1 ? 's' : ''} de la lista`);
      setRowSelection({});
      router.refresh();
    } catch (error) {
              ...
              <DropdownMenuContent align="end">
                ...
                {canDelete && (
                  <>
                    <DropdownMenuSeparator />
                    <DropdownMenuItem
                      onClick={() => setDeleteTarget({ id: item.id, name: item.product?.name || 'el ítem' })}
  ...
  return (
    <>
      ...
      <AlertDialog open={bulkDeleteOpen} onOpenChange={setBulkDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Eliminar {selectedCount} ítem{selectedCount !== 1 ? 's' : ''}</AlertDialogTitle>
            <AlertDialogDescription>
              Vas a eliminar {selectedCount} ítem{selectedCount !== 1 ? 's' : ''} de esta lista de precios. Esta acción no se puede deshacer.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
