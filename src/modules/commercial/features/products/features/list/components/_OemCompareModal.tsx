    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-4xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Ingresá un código OEM para comparar precios entre ítems equivalentes de distintos proveedores.
          </DialogDescription>
        </DialogHeader>
              ...
              {!isLoading && data && (
                <p className="text-sm text-muted-foreground mb-2">
                  {data.products.length === 0
                    ? `No se encontraron ítems con código OEM "${data.oemCode}"`
                    : `${data.products.length} ítem${data.products.length !== 1 ? 's' : ''} encontrado${data.products.length !== 1 ? 's' : ''} para OEM "${data.oemCode}"`}
                </p>
              )}
